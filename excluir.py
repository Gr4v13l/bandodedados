...
sql = 'SELECT * FROM Brasil.tribos WHERE nome_tribo LIKE %s'
val = ('%'+busca+'%',)
...
if result:
    ...
    if confirmacao.lower() == 's':
        sql = 'DELETE FROM Brasil.tribos WHERE nome_tribo = %s'
        val = (nome_tribo,)
        cursor.execute(sql,val)
        conn.commit()
 ...
 print('Não foi encontrado nenhuma tribo com o nome informado.')
